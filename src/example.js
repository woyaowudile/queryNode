   ...
   let next = function () {
       if (count < 1) {
           ...
           setTimeout( async () => {
               ...
               const res1 = await getApi(code, 'real/time', type[dwmType])
...
function init() {
    ...
    let next = function () {
        ...
        setTimeout( async () => {
            ...
            if (res1.code === 200) {
                // 3.1 将成功的code存储
                let sql = `INSERT INTO ig502_used(code, type, type1) VALUES(${code}, ${codeName.split('_')[2]}, '${dwmType}')`
